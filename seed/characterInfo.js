...
const main = async () => {
  ...
  const blackClover = await AnimeName.findOne({name: 'Black Clover'})
  const jujutsuKaisen = await AnimeName.findOne({name: 'Jujutsu Kaisen'})
  const onePiece = await AnimeName.findOne({name: 'One Piece'})
  ...
  const characterInfo = [
    ...
    },
    {
      ...
      animeName_id: jujutsuKaisen._id
    },
    {
      name: 'Satoru Gojo',
